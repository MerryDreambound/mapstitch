...
public class ModUtil {
	...
	public static boolean hasCompass(Minecraft mc) {
		...
		if (!bundles.isEmpty()) for (BundleContents bundle : bundles) {
			for (ItemStackTemplate stack : bundle.items()) if (stack.is(Items.COMPASS)) return true;
		}
		if (!containers.isEmpty()) for (ItemContainerContents container : containers) {
			for (ItemStackTemplate stack : container.nonEmptyItems()) if (stack.is(Items.COMPASS)) return true;
		}
		return false;
	}
}
